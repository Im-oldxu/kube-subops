...
interface Props {
  ...
  options: ReadonlyArray<SelectOption> | ReadonlyArray<Record<string, unknown>>
  placeholder?: string
  disabled?: boolean
  error?: boolean
  searchable?: boolean
...
// i18n placeholders
const placeholderText = computed(() => props.placeholder ?? defaultTexts.selectOption)
const searchPlaceholderText = computed(() => props.searchPlaceholder ?? defaultTexts.searchPlaceholder)
const emptyTextDisplay = computed(() => props.emptyText ?? defaultTexts.noOptionsFound)
...
const filteredOptions = computed(() => {
  let opts = [...(props.options as readonly any[])]
  if (props.searchable && searchQuery.value) {
    ...
    if (props.creatable && searchQuery.value.trim()) {
      ...
      const prefix = props.creatablePrefix || defaultTexts.search
